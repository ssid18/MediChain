...
type DoctorSignUpFormData = DoctorRegistrationData & {
...
const DoctorSignUpPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        let hasErrors = false;
        (Object.keys(formData) as Array<keyof DoctorSignUpFormData>).forEach(key => {
        ...
        try {
            ...
            await authContext?.registerDoctor(registrationData);
    ...
    const fillWithDemoData = () => {
        setFormData({
            ...
            email: 'dr.demo@medichain.com',
            physicalAddress: '123 Demo Street, Health City',
        });
        setErrors({});
    };
        ...
        <div className="w-full max-w-2xl mx-auto animate-fade-in">
          <div className="bg-slate-800/50 p-8 rounded-xl shadow-2xl border border-slate-700/50">
            <div className="text-center mb-8">
                <h1 className="text-3xl font-bold tracking-tight text-white mb-2">Doctor Verification</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  <label htmlFor="clinicName" className="block text-sm font-medium text-slate-300 mb-1">Clinic/Hospital Name</label>
                  <input type="text" name="clinicName" id="clinicName" value={formData.clinicName} onChange={handleChange} onBlur={handleBlur} className={inputClass('clinicName')} required />
                  {errors.clinicName && <p className="mt-1 text-xs text-red-400">{errors.clinicName}</p>}
                </div>
                 <div>
                  <label htmlFor="licenseNumber" className="block text-sm font-medium text-slate-300 mb-1">Medical License No.</label>
                  <input type="text" name="licenseNumber" id="licenseNumber" placeholder="e.g., 12345678" value={formData.licenseNumber} onChange={handleChange} onBlur={handleBlur} className={inputClass('licenseNumber')} required />
